...
std::optional<std::string> HttpMessage::getHeaderValue(const std::string& headerName) const {
...
HttpMethod HttpMessage::stringToHttpMethod(const std::string& methodStr) {
...
std::string HttpMessage::httpMethodToString(const HttpMethod& method) {
	switch (method) {
	case HttpMethod::GET:
		return "GET";
		break;
...
void HttpMessage::parseRequest(const std::string& raw) {
	...
	this->mMethod = HttpMessage::stringToHttpMethod(requestMethodStr);
